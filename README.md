...
* The [Simplicial Complex](https://en.wikipedia.org/wiki/Simplicial_complex) encoder works for any $n$ and $m$ and encodes into $n*m+1$ reals. ([encoder source](C0_numpy_simplicalComplex_encoder_for_array_of_reals_as_multiset.py))
...
* The polynomial encoders are efficient (i.e. encode into $nm$ reals) for $m=1$ or $m=2$ but in general have order $O(n m^2)$. (encoder sources ([for arrays](Cinf_numpy_polynomial_encoder_for_array_of_reals_as_multiset.py)) and ([for list of reals](Cinf_numpy_polynomial_encoder_for_list_of_reals_as_multiset.py)))
